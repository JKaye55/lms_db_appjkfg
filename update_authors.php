...
if(empty($id = $_POST['id'])) {
 ...
 if (isset($_POST['update'])) {
  $authorId= $_POST['id'];
  $authorfirst = $_POST['authorFirstName'];
  $authorlast = $_POST['authorLastName'];
  $authorbday = $_POST['authorBirthYear'];
  $authornat = $_POST['authorNationality'];

$authorupID= $con->UpdateAuthors($authorfirst, $authorlast, $authorbday, $authornat, $authorId);
...
<div class="container my-5 border border-2 rounded-3 shadow p-4 bg-light">
  ...
  <h4 class="mt-5">Updating Existing Author</h4>
    <form id="authorForm" method="POST" action="">
         <div class="mb-3">
      <label for="author_ID" class="form-label"></label>
      <input type="hidden"  name="id" value="<?php echo $data['author_id']?>" class="form-control" id="author_ID" required>
    </div>
    <div class="mb-3">
      <label for="authorFirstName" class="form-label">First Name</label>
      <input type="text"  name="authorFirstName" value="<?php echo $data['author_FN']?>" class="form-control" id="authorFirstName" required>
    </div>
    <div class="mb-3">
      <label for="authorLastName"  class="form-label">Last Name</label>
      <input type="text"  name="authorLastName"   value="<?php echo $data['author_LN']?>" class="form-control" id="authorLastName" required>
    </div>
    <div class="mb-3">
      <label for="authorBirthYear" class="form-label">Birth Date</label>
      <input type="date"  name="authorBirthYear" value="<?php echo $data['author_birthday']?>" class="form-control" id="authorBirthYear" max="<?= date('Y-m-d') ?>" required>
    </div>
    <div class="mb-3">
      <label for="authorNationality"  class="form-label">Nationality</label>
      <select class="form-select" name="authorNationality" value="<?php echo $data['author_nat']?>" id="authorNationality" required>
